--[[
#---------------------------------------------------------------#
----* DayZ MTA Script gang_utils.lua                        *----
...
addGang = function(gangName, gangLeader)
  if not doesGangExists(gangName) then
    if executeSQLInsert("gangs", "'" .. tostring(gangName) .. "','" .. tostring(gangLeader) .. "','0','[ ]'") then
      outputDebugString("Group System: group name: " .. tostring(gangName) .. ", leader: " .. tostring(gangLeader))
...
getGangMembers = function(gangName)
  if doesGangExists(gangName) then
    ...
    return false, "The group doesn't exists."
  end
end
...
addGangMember = function(gangName, memberAccount, addedBy)
  ...
  if executeSQLInsert("gang_members", "'" .. tostring(gangName) .. "','" .. tostring(memberAccount) .. "','" .. tostring(addedBy) .. "'") then
    ...
    outputDebugString("Group System: Member added to: " .. tostring(gangName) .. ", account: " .. tostring(memberAccount))
    return true
  else
    return false, "An error has occurred."
  end
    ...
    if account and not isGuestAccount(account) then
      ...
      if player then
        if player and kickerName then
          outputChatBox("Group System: You have been kicked from " .. tostring(gangName) .. " by " .. tostring(kickerName) .. ".", player, 255, 0, 0)
        else
          outputChatBox("Group System: You have left " .. tostring(gangName) .. ".", player, 255, 0, 0)
        end
