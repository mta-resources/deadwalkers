...
function aSpectate ( player )
	if ( player == getLocalPlayer() ) then
		aMessageBox ( "error", "Você não pode visualizar a si mesmo(a)." )
		return
	end
...
function aSpectator.Initialize ()
	if ( aSpectator.Actions == nil ) then
		...
		aSpectator.Kick			= guiCreateButton ( 0.10, 0.15, 0.80, 0.05, "Kickar", true, aSpectator.Actions )
		aSpectator.Freeze			= guiCreateButton ( 0.10, 0.21, 0.80, 0.05, "Congelar", true, aSpectator.Actions )
		--[[
		aSpectator.SetSkin		= guiCreateButton ( 0.10, 0.27, 0.80, 0.05, "Def. Skin", true, aSpectator.Actions )
		aSpectator.SetHealth		= guiCreateButton ( 0.10, 0.33, 0.80, 0.05, "Def. Sangue", true, aSpectator.Actions )
		aSpectator.SetArmour		= guiCreateButton ( 0.10, 0.39, 0.80, 0.05, "Def. Colete", true, aSpectator.Actions )
		aSpectator.SetStats		= guiCreateButton ( 0.10, 0.45, 0.80, 0.05, "Def. Status", true, aSpectator.Actions )
		...
		aSpectator.Skip			= guiCreateCheckBox ( 0.08, 0.85, 0.84, 0.04, "Pular mortos", true, true, aSpectator.Actions )
		...
		aSpectator.PlayerList		= guiCreateGridList ( 0.03, 0.07, 0.94, 0.92, true, aSpectator.Players )
					  		  guiGridListAddColumn( aSpectator.PlayerList, "Nome do jogador", 0.85 )
...
function aSpectator.SwitchPlayer ( inc, arg, inc2 )
	...
	if ( #players <= 0 ) then
		aMessageBox ( "question", "Ninguém para visualizar, sair do visualizador?", "aSpectator.Close ( false )" )
		return
	end
	local current = 1
	...
	if ( next == current ) then
		aMessageBox ( "question", "Mais ninguém para visualizar, sair do visualizador?", "aSpectator.Close ( false )" )
		return
	end
