...
function aBanDetails ( ip )
	if ( aBanForm == nil ) then
		...
		aBanForm		= guiCreateWindow ( x / 2 - 130, y / 2 - 150, 260, 300, "Banimentos", false )
	...
	if ( aBans["IP"][ip] ) then
		...
		guiSetText ( aBanTime, "Hora: "..iif ( aBans["IP"][ip]["time"], aBans["IP"][ip]["time"], "Desconhecido" ) )
		...
		aBanReason = guiCreateLabel ( 0.03, 0.60, 0.80, 0.30, "Motivos: "..iif ( aBans["IP"][ip]["reason"], aBans["IP"][ip]["reason"], "Desconhecido" ), true, aBanForm )
	...
	elseif ( aBans["Serial"][ip] ) then
		...
		guiSetText ( aBanTime, "Hora: "..iif ( aBans["Serial"][ip]["time"], aBans["Serial"][ip]["time"], "Desconhecido" ) )
