...
function aInputBox ( title, message, default, action )
	if ( aInputForm == nil ) then
		...
		aInputOk		= guiCreateButton ( 90, 80, 60, 17, "Ok", false, aInputForm )
		aInputCancel		= guiCreateButton ( 155, 80, 60, 17, "Cancelar", false, aInputForm )
...
function aBanInputBox ( player )
	...
	if ( aBanInputForm == nil ) then
		...
		aBanInputOk			= guiCreateButton ( 90, y, 60, 17, "Ok", false, aBanInputForm )
		aBanInputCancel		= guiCreateButton ( 155, y, 60, 17, "Cancelar", false, aBanInputForm )
		y = y + 30
	...
	for i,dur in ipairs(aBanDurations) do
		if dur == "Custom:" then
		guiSetText ( aBanInputRadio2s[i], "Person.:" )
		else
	...
	guiSetText ( aBanInputLabel, "Digite o motivo" )
	aHideFloaters()
...
function aBanInputBoxFinish ()
	...
	if not bUseIP and not bUseSerial then
		aMessageBox ( "error", "Nenhum tipo selecionado!" )
		return
	end
...
function aMuteInputBox ( player )
	...
	if ( aMuteInputForm == nil ) then
		...
		local yy = 5
		aMuteInputRadio2Label	= guiCreateLabel ( 10, yy, 270, 15, "Duração:", false, aMuteInputRadioSet2 )
		aMuteInputRadio2s = {}
		...
		aMuteInputOk			= guiCreateButton ( 90, y, 60, 17, "Ok", false, aMuteInputForm )
		aMuteInputCancel		= guiCreateButton ( 155, y, 60, 17, "Cancelar", false, aMuteInputForm )
		y = y + 30
	...
	-- update duration values in the form
	for i,dur in ipairs(aMuteDurations) do
		guiSetText ( aMuteInputRadio2s[i], dur>0 and secondsToTimeDesc(dur) or "Até reconectar" )
	end

	guiSetInputEnabled ( true )
	guiSetText ( aMuteInputForm, "Mutar jogador " .. getPlayerName(player) )
	guiSetText ( aMuteInputLabel, "Digite o motivo" )
	aHideFloaters()
